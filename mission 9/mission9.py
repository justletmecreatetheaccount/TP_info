...
class ArticleReparation(Article):
    def __init__(self, d, duration, prix_fixe = 20, prix_variable = 10):
        super().__init__(d, prix_fixe + prix_variable * duration)
        #if type(duration) != 'float' or type(duration) != 'int':
         #   raise TypeError("Time must be recorded as a number") 
        self.duration = duration

    def description(self):
        return super().description() + ' ({} heures)'.format(self.duration)
